...
# TODO: fix issue with file path
paths <- dir(
  path = system.file("extdata", "CDOMtools", mustWork = TRUE),
  pattern = "\\.txt$", 
  full.names = TRUE)
...
Here is an example for calculating the log-transformed spectral slope in the 275-295 nm range ($S_{275-295}$), a common proxy for DOM molecular weight.
...
nested_cdom <- nested_cdom %>% 
  mutate(SUVA254 = map_dbl(data,
                          ~{calc_SUVA(wavelength = .$wavelength,
                                     absorption = .$absorption,
                                     DOC = DOC,
